...
- 知识追踪：`kt` + TCN 图谱。接入口径见 [TCN集成.md](./TCN集成.md)（题 tag 来自词表，交卷 `predict`，不接 Learning OS）
- 搜索：`search`
...
- **进度**：看自己怎么样（热力、正确率、薄弱点 → 针对训练）。章维度的对/错/不会、题量也放这里，点开一本书再细看。不是任务历史。
- **学习路径**：下一步按什么顺序走。有 `tcn_domain` 时主画面是该科知识图（先修 + 掌握度）和一条「现在补这个」；没有学科时用目录当顺序条，只标当前章，不写题量和对错。点章或节点去出题/刷题，不要画成任务 DAG，也不要把记分榜搬过来。
