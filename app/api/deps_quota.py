...
def _apply_quota(current_user: dict, db: Session) -> None:
    """按套餐检查日调用 / 月 token；超限抛 429。QUOTA_ENFORCE=false 时直接返回。"""
...
def check_quota(
    ...
    """
    _apply_quota(current_user, db)
    return current_user


def enforce_quota_for_user(current_user: dict) -> None:
    """SSE 用：配额检查自管短 Session，不经过 Depends(get_db)。"""
    if not QUOTA_ENFORCE:
        return
